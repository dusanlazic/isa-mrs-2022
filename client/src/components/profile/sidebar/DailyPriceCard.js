    ...
    return (
      ...
      shadow-sm py-3 px-5 tracking-wide">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-y-3">
          {data.prices.map(tag => 
          <div key={tag.id}>
            <h1 className="text-sm leading-5">Price per day for more than {tag.minDays} days</h1>
            <span className="font-mono font-bold text-lg leading-4">{tag.value}€</span>
          </div>  
          )}
        </div>
